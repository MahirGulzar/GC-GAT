...
class NuScenesVector(NuScenesTrajectories):
    ...
    def get_target_agent_representation(self, idx: int) -> np.ndarray:
        ...
        motion_states = self.get_past_motion_states(i_t, s_t, self.t_h)
    ...
    def get_polygons_around_agent(self, global_pose: Tuple[float, float, float], map_api: NuScenesMap, polygon_types:list) -> Dict:
        """
        ...
        x, y, _ = global_pose
        radius = max(self.map_extent)
        record_tokens = map_api.get_records_in_radius(x, y, radius, polygon_types)
    ...
    def get_agents_of_type(self, idx: int, agent_type: str) -> List[np.ndarray]:
        ...
        for n, agent in enumerate(agent_list):
            ...
            motion_states = self.get_past_motion_states(agent_i_ts[n], s_t, self.t_h)
    ...
    def get_past_motion_states(self, i_t, s_t, t_steps):
        """
